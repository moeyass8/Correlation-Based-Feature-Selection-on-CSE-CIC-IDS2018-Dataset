...
def preprocess(DatasetLocation):
	...
	df = pd.DataFrame(pd.read_csv(DatasetLocation))  #this will change the csv dataset to pandas dataframe
	...
	features = features.replace([np.inf, -np.inf], np.nan) #changing the infs to nans
	#print(features.info())
	#input("stoppppp")
	features = features.fillna(0.0) #changing the nans to 0s 
